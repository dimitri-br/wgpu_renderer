...
impl LightManager {
    ...
    pub fn create_directional_light(&mut self, shadow_map_resolution: u32, cast_shadow: bool, shadow_atlas: &mut ShadowAtlas) -> Light {
        ...
        let mut light = Light::new(position, direction, color, intensity, range, spot_angle, cast_shadow, LightType::Directional);
    ...
    pub fn create_point_light(
        ...
        let mut light = Light::new(position, rotation, color, intensity, range, 0.0, cast_shadow, LightType::Point);
    ...
    pub fn create_spot_light(
        ...
        let mut light = Light::new(position, direction, color, intensity, range, spot_angle, cast_shadow, LightType::Spot);
    ...
    pub fn update_lights(&mut self, lights: &mut [Light], camera: &Box<dyn Camera>, shadow_atlas: &mut ShadowAtlas) {
        for (idx, light) in lights.iter_mut().enumerate() {
            if let Some(cached_light) = self.light_cache.get(idx) {
                if (light.position == cached_light.position || light.rotation == cached_light.rotation) && light.light_type != LightType::Directional {
                    continue;
                }
            }
            match LightType::from_u32(light.light_type) {
                LightType::Directional => {
                    Self::update_directional_light(light, camera, &mut self.shadow_data_storage, shadow_atlas)
                },
                LightType::Point => {
                    Self::update_point_light(light, &mut self.shadow_data_storage, shadow_atlas)
                },
                LightType::Spot => {
                    Self::update_spot_light(light, &mut self.shadow_data_storage, shadow_atlas)
                },
                _ => {}
            }
        }
    ...
    fn update_directional_light(light: &mut Light, camera: &Box<dyn Camera>, shadow_storage: &mut ShadowDataStorage, shadow_atlas: &mut ShadowAtlas) {
        ...
        if light.cast_shadow != 0 {
            // If needed, create a new shadow tile.
            if light.shadow_data_count == 0 {
                let tile = shadow_atlas
                ...
                let smc = crate::renderer::ecs::components::ShadowMapComponent::new(shadow_data, tile);
                let idx = shadow_storage.add_shadow_data(smc);
                light.set_shadow_data(idx as u32, 1);
            }
    ...
    fn update_point_light(light: &mut Light, shadow_storage: &mut ShadowDataStorage, shadow_atlas: &mut ShadowAtlas) {
    ...
    fn update_spot_light(light: &mut Light, shadow_storage: &mut ShadowDataStorage, shadow_atlas: &mut ShadowAtlas) {
        let light_pos = light.position;
        let light_dir = light.rotation.normalize();
